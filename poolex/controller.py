...
Protocole observé avr 2026 :
  ...
  Remote → PAC : CC — réponse keepalive après chaque D2 (miroir du D2 reçu)
                 CD — commande de changement (setpoint, power) → burst de 3

  D2/CC/CD byte[11]      → consigne température °C
  D2/CC/CD byte[1] bit 0 → état on/off  (0=arrêt, 1=allumage)
  D2/CC/CD byte[1] bits 4-6 → mode (0x1b=inverter, 0x3b=fix, 0x5b=full ?)
  DD byte[3]             → état PAC (0xa1=chauffe, 0x21=standby,
    ...
    def __init__(self, capture: RS485Capture, storage: "Storage | None" = None) -> None:
        ...
        if storage is not None:
            ...
            if d2 is not None:
                ...
                logger.info(
                    "Template chargé depuis DB: consigne=%d°C power=%s b1=0x%02x",
